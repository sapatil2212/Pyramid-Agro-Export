...
export function HeroSlideForm({ isNew, initialData, onSave, onCancel, saving }: HeroSlideFormProps) {
  ...
  return (
    <Card className="p-3 md:p-5 border-2 border-emerald-200 bg-gradient-to-br from-emerald-50/50 to-white shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
            {isNew ? <Plus className="h-4 w-4 text-emerald-600" /> : <Edit className="h-4 w-4 text-emerald-600" />}
          </div>
          <h4 className="text-sm md:text-base font-semibold text-gray-900">{isNew ? 'Add New Hero Slide' : 'Edit Hero Slide'}</h4>
        </div>
                ...
                {uploading && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                    <div className="bg-white rounded-lg p-1.5 md:p-2 flex items-center gap-2">
                      <LoadingSpinner size="sm" /><span className="text-xxs md:text-xs">Uploading...</span>
                    </div>
                  </div>
                )}
              </div>

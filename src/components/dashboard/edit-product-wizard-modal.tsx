...
interface EditProductWizardModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export function EditProductWizardModal({ 
  isOpen, 
  onClose, 
  onSuccess, 
  product,
  ...
  const [editingTableTitle, setEditingTableTitle] = useState<number | null>(null)
  const [editTableTitleValue, setEditTableTitleValue] = useState("")
  ...
  // Track if data has been loaded to prevent re-loading during editing
  const [dataLoaded, setDataLoaded] = useState(false)

  // Load product data when modal opens
  useEffect(() => {
    if (isOpen && product && !dataLoaded) {
  ...
  const loadProductData = (prod: Product) => {
    ...
    setHeroData({
      name: prod.name || "",
      slug: prod.slug || "",
      heroTitle: prod.heroTitle || "",
      heroSubtitle: prod.heroSubtitle || "",
      heroDescription: prod.heroDescription || "",
      heroImageUrl: prod.heroImageUrl || prod.imageUrl || "",
      heroButtonText: prod.heroButtonText || "Get Quote",
      heroButtonLink: prod.heroButtonLink || "/contact",
      heroButton2Text: prod.heroButton2Text || "Learn More",
      heroButton2Link: prod.heroButton2Link || "#features"
    })
    
    // Load features data
    const productFeatures = prod.features || []
    const normalizedFeatures: Feature[] = (productFeatures as Array<Feature | string>).map((f) => {
      if (typeof f === 'string') {
        return { icon: "Star", title: f, description: "" }
      }
      return {
        icon: f.icon || "Star",
        title: f.title || "",
    ...
    setFeaturesData({
      featuresTitle: prod.featuresTitle || "",
      featuresSubtitle: prod.featuresSubtitle || "",
      features: normalizedFeatures
    })
    
    // Load specification tables
    const tables: SpecTable[] = []
    
    // First, try to load from tableSpecs (new format with multiple tables)
    if (prod.tableSpecs) {
      try {
        const parsedTables = typeof prod.tableSpecs === 'string' 
          ? JSON.parse(prod.tableSpecs) 
          : prod.tableSpecs
    ...
    // Fallback: Load from specifications (old format - single table)
    if (tables.length === 0 && prod.specifications && Object.keys(prod.specifications).length > 0) {
      tables.push({
        title: "Product Specifications",
        rows: Object.entries(prod.specifications).map(([key, value]) => ({ key, value }))
      })
    }
    
    console.log('Loading specification tables:', tables.length, tables)
    setSpecTables(tables)
    
    // Load CTA data
    setCtaData({
      ctaTitle: prod.ctaTitle || "",
      ctaDescription: prod.ctaDescription || "",
      ctaButtonText: prod.ctaButtonText || "Get Quote Now",
      ctaButtonLink: prod.ctaButtonLink || "/contact"
    })
  }
  ...
  const handleNameChange = (name: string) => {
    setHeroData(prev => ({
      ...
      heroTitle: name && !prev.heroTitle ? `Premium Quality ${name}` : prev.heroTitle
    }))
  }
  ...
  const addTable = () => {
    ...
    const newTable = { title: newTableTitle, rows: [] }
    setSpecTables(prev => {
      const updated = [...prev, newTable]
      console.log('Adding new table. Total tables:', updated.length)
      return updated
    })
  ...
  const startEditRow = (tableIndex: number, rowIndex: number) => {
    const row = specTables[tableIndex].rows[rowIndex]
    setEditingRow({ tableIndex, rowIndex })
    setEditRowKey(row.key)
    setEditRowValue(row.value)
  }

  const saveEditRow = () => {
    if (!editingRow || !editRowKey.trim() || !editRowValue.trim()) {
      toast.error("Row key and value required")
      return
    }
    
    setSpecTables(prev => prev.map((table, i) => 
      i === editingRow.tableIndex 
        ? { 
            ...table, 
            rows: table.rows.map((row, ri) => 
              ri === editingRow.rowIndex 
                ? { key: editRowKey, value: editRowValue } 
                : row
            ) 
          } 
        : table
  ...
  const startEditTableTitle = (tableIndex: number) => {
    const table = specTables[tableIndex]
    setEditingTableTitle(tableIndex)
    setEditTableTitleValue(table.title)
  }

  const saveEditTableTitle = () => {
    if (!editTableTitleValue.trim()) {
      toast.error("Table title is required")
      return
    }
    
    setSpecTables(prev => prev.map((table, i) => 
      i === editingTableTitle ? { ...table, title: editTableTitleValue } : table
    ))
    
    setEditingTableTitle(null)
    setEditTableTitleValue("")
    toast.success("Table title updated")
  }

  const cancelEditTableTitle = () => {
    setEditingTableTitle(null)
    setEditTableTitleValue("")
  }
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      // Save all specification tables as JSON array
      // For backward compatibility, if there's only one table, also save as flat object
      const specsObj: Record<string, string> = {}
      if (specTables.length > 0 && specTables[0].rows.length > 0) {
        specTables[0].rows.forEach(row => { specsObj[row.key] = row.value })
      }
      
      // Store all tables in a structured format
      const allTables = specTables.map(table => ({
        title: table.title,
        rows: table.rows
      }))
      
      console.log('Saving specification tables:', allTables.length, allTables)
      ...
      const productData = {
        ...
        heroButton2Text: heroData.heroButton2Text, 
        heroButton2Link: heroData.heroButton2Link,
        ...
        specifications: specsObj, // Keep for backward compatibility
        // Store all tables in tableSpecs field (will be stringified by API)
        tableSpecs: allTables.length > 0 ? allTables : null,
    ...
    <Dialog open={isOpen && !showDeleteConfirm} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto p-4">
        <DialogHeader className="pb-2">
          ...
          <DialogDescription className="text-[11px] text-center">Update product details in a few simple steps</DialogDescription>
        </DialogHeader>
                ...
                {specTables.map((table, tableIndex) => (
                  <div key={tableIndex} className={cn("border rounded-lg overflow-hidden", activeTableIndex === tableIndex ? "ring-2 ring-emerald-500" : "")}>
                    ...
                    <div className="bg-emerald-600 text-white px-3 py-2 flex items-center justify-between">
                      {editingTableTitle === tableIndex ? (
                        // Edit mode
                        <div className="flex items-center gap-2 flex-1">
                          <Table className="h-3.5 w-3.5 flex-shrink-0" />
                          <Input 
                            className="h-7 text-xs flex-1 bg-white text-gray-900" 
                            value={editTableTitleValue} 
                            onChange={(e) => setEditTableTitleValue(e.target.value)}
                            placeholder="Table title"
                            autoFocus
                          />
                          <button 
                            onClick={saveEditTableTitle} 
                            className="text-white hover:text-emerald-200 flex-shrink-0"
                            title="Save"
                          >
                            <Check className="h-4 w-4" />
                          </button>
                          <button 
                            onClick={cancelEditTableTitle} 
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-1">
                            <button 
                              onClick={(e) => {
                                e.stopPropagation()
                                startEditTableTitle(tableIndex)
                              }} 
                        ...
                        {table.rows.length > 0 ? (
                          <div className="space-y-1">
                            {table.rows.map((row, rowIndex) => {
                              const isEditing = editingRow?.tableIndex === tableIndex && editingRow?.rowIndex === rowIndex
                              
                              if (isEditing) {
                                return (
                                  <div key={rowIndex} className="flex items-center gap-2 text-xs bg-emerald-50 border border-emerald-200 rounded px-2 py-1.5">
                                    <Input 
                                      className="h-7 text-xs w-1/3" 
                                      value={editRowKey} 
                                      onChange={(e) => setEditRowKey(e.target.value)}
                                      placeholder="Key"
                                    />
                                    <Input 
                              ...
                              return (
                                <div key={rowIndex} className="flex items-center gap-2 text-xs bg-gray-50 rounded px-2 py-1.5">
                                  ...
                                  <button 
                                    onClick={() => startEditRow(tableIndex, rowIndex)} 
                                    className="text-emerald-600 hover:text-emerald-700 flex-shrink-0"
                                    title="Edit row"
                                  >
                                    <Edit className="h-3.5 w-3.5" />

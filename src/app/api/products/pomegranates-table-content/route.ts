...
// PUT /api/products/pomegranates-table-content - Update table content for pomegranates product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the pomegranates product
    const pomegranatesProduct = await prisma.product.findFirst({
      where: { slug: "pomegranates" }
    })

    if (!pomegranatesProduct) {
      return NextResponse.json({ error: "Pomegranates product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: pomegranatesProduct.id },
      data: {

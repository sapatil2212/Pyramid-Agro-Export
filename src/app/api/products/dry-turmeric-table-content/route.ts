...
// PUT /api/products/dry-turmeric-table-content - Update table content for dry-turmeric product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the dry-turmeric product
    const dryturmericProduct = await prisma.product.findFirst({
      where: { slug: "dry-turmeric" }
    })

    if (!dryturmericProduct) {
      return NextResponse.json({ error: "Dry Turmeric product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: dryturmericProduct.id },
      data: {

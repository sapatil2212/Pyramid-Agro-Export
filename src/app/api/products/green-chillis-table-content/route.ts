...
// PUT /api/products/green-chillis-table-content - Update table content for green-chillis product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the green-chillis product
    const greenchillisProduct = await prisma.product.findFirst({
      where: { slug: "green-chillis" }
    })

    if (!greenchillisProduct) {
      return NextResponse.json({ error: "Green Chilies product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: greenchillisProduct.id },
      data: {

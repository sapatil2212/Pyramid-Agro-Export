...
// PUT /api/products/rice-table-content - Update table content for rice product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the rice product
    const riceProduct = await prisma.product.findFirst({
      where: { slug: "rice" }
    })

    if (!riceProduct) {
      return NextResponse.json({ error: "Basmati Rice product not found" }, { status: 404 })
    }

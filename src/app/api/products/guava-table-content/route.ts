...
// PUT /api/products/guava-table-content - Update table content for guava product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the guava product
    const guavaProduct = await prisma.product.findFirst({
      where: { slug: "guava" }
    })

    if (!guavaProduct) {
      return NextResponse.json({ error: "Guavas product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: guavaProduct.id },
      data: {

...
// PUT /api/products/garlics-table-content - Update table content for garlics product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the garlics product
    const garlicsProduct = await prisma.product.findFirst({
      where: { slug: "garlics" }
    })

    if (!garlicsProduct) {
      return NextResponse.json({ error: "Garlics product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: garlicsProduct.id },
      data: {

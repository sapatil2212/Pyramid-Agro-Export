...
// PUT /api/products/table-content - Update table content for grapes product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the grapes product
    const grapesProduct = await prisma.product.findFirst({
      where: { slug: "grapes" }
    })

    if (!grapesProduct) {
      return NextResponse.json({ error: "Grapes product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: grapesProduct.id },
      data: {

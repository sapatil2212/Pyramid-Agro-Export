...
// PUT /api/products/tomatoes-table-content - Update table content for tomatoes product
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find the tomatoes product
    const tomatoesProduct = await prisma.product.findFirst({
      where: { slug: "tomatoes" }
    })

    if (!tomatoesProduct) {
      return NextResponse.json({ error: "Tomatoes product not found" }, { status: 404 })
    }

    // Update the product with new table content
    const updatedProduct = await prisma.product.update({
      where: { id: tomatoesProduct.id },
      data: {
